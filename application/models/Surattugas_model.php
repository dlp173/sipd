...
function getSurattugas($search){
        ...
        if ($query->num_rows() > 0) { //jika ada maka jalankan
            ...
            $this->db->select('tsurattugas.kegiatan_id as id, tpegawai.nama as nama, tpegawai.nip as nip,tgolongan.gol_ruang as gol_ruang,tgolongan.golongan as golongan, tpegawai.jabatan as jabatan');
            $this->db->from('tsurattugas');
            $this->db->join('tkegiatan', 'tkegiatan.id = tsurattugas.kegiatan_id','inner');
            $this->db->join('tpegawai','tsurattugas.pegawai_id = tpegawai.id','inner');
            $this->db->join('tgolongan','tgolongan.id = tpegawai.golongan_id','inner');
            $this->db->where('tsurattugas.kegiatan_id',$id);
...
function printtgskegiatan($id){
    $this->db->distinct();
    $this->db->select('tkegiatan.jeniskegiatan,tkegiatan.tgl_pergi,tkegiatan.tgl_pulang,tkegiatan.tempat,tkegiatan.no_lampiran');
    $this->db->from('tsurattugas');
